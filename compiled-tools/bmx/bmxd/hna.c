...
//stephan; ignore originator if it uses hna 
		...
		{
			dbg_mute( 45, DBGL_SYS, DBGT_WARN, "NOT ALLOWED to send HNA -> drop OGM: originator %15s ", orig_node->orig_str);
			      ...
			      key.addr != ( key.addr & htonl( 0xFFFFFFFF<<(32 - key.KEY_FIELD_ANETMASK ) ) ) ) 
			{
				...
				return CB_OGM_REJECT;
                                ...
                                dbg_mute(45, DBGL_SYS, DBGT_WARN,
                                        ...
                                        (hn->status == HNA_HASH_NODE_OTHER ?
                                        (int) (dad_to - (((batman_time_t) (batman_time - hn->orig->last_valid_time)) / 1000)) :
                                        -1),
                                        ARG_PURGE_TO, dad_to);
